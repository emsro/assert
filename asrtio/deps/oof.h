...
[[nodiscard]] auto fg_color( color const& col ) -> fg_rgb_color_sequence;

// Sets the foreground indexed color. Index must be in [1, 255]. You can define colors with
// set_index_color().
[[nodiscard]] auto fg_color( int index ) -> fg_index_color_sequence;

// Sets the background RGB color
[[nodiscard]] auto bg_color( color const& col ) -> bg_rgb_color_sequence;

// Sets the background indexed color. Index must be in [1, 255]. You can define colors with
// set_index_color().
[[nodiscard]] auto bg_color( int index ) -> bg_index_color_sequence;

// Sets the indexed color. Index must be in [1, 255].
[[nodiscard]] auto set_index_color( int index, color const& col ) -> set_index_color_sequence;
...
using error_callback_type                 = void ( * )( std::string const& msg );
...
auto write_sequence_into_string( string_type& target, sequence_type const& sequence ) -> void;

// Returns a sing from a sequence type
template < oof::std_string_type string_type, oof::sequence_c sequence_type >
[[nodiscard]] auto get_string_from_sequence( sequence_type const& sequence ) -> string_type;
...
[[nodiscard]] auto get_string_from_sequences(
    std::vector< sequence_variant_type > const& sequences ) -> string_type;

// Returns the exact size a string from this vector of sequence types
[[nodiscard]] auto get_string_reserve_size( std::vector< sequence_variant_type > const& sequences )
    -> size_t;
...
struct cell_format
{
        ...
        friend constexpr auto operator==( cell_format const&, cell_format const& )
            -> bool = default;
};
...
struct cell
{
        ...
        friend constexpr auto operator==( cell const&, cell const& ) -> bool = default;
};
...
struct screen
{
        ...
        explicit screen(
            ...
            int                        start_column,
            int                        start_line,
            cell< string_type > const& background );
        ...
        // This constructor taking a fill_char implies black background, white foreground color and
        // top left start
        ...
        auto write_into(
            ...
            int                column,
            int                line,
            cell_format const& formatting ) -> void;
...
struct pixel_screen
{
        ...
        [[nodiscard]] auto get_color( int column, int halfline ) const -> color const&;
...
private:
        ...
        int   m_halfline_height =
            0;  // This refers to "pixel" height. Height in lines will be half that.
        int                            m_origin_column   = 0;
...
auto operator<<( stream_type& os, sequence_type const& sequence ) -> stream_type&;

namespace detail
{

// CRTP to extend the numerous sequence types with convenience member functions without using
// runtime polymorphism or repeating the code
template < typename T >
struct extender
{
        operator std::string() const;
        operator std::wstring() const;
        [[nodiscard]] auto operator+( std::string const& other ) const -> std::string;
        [[nodiscard]] auto operator+( std::wstring const& other ) const -> std::wstring;
};

auto error( std::string const& msg ) -> void;

[[nodiscard]] auto get_pixel_background( color const& fill_color ) -> cell< std::wstring >;

template < oof::std_string_type string_type >
auto write_sequence_string_no_reserve(
    std::vector< sequence_variant_type > const& sequences,
    string_type&                                target ) -> void;

template < oof::sequence_c sequence_type >
[[nodiscard]] constexpr auto get_sequence_string_size( sequence_type const& sequence ) -> size_t;

template < oof::std_string_type string_type, std::integral int_type >
auto write_int_to_string(
    string_type&   target,
    int_type const value,
    bool const     with_leading_semicolon ) -> void;
...
struct cell_pos
{
        ...
        explicit constexpr cell_pos( int const width, int const height )
          : m_width( width )
          , m_height( height )
        {
        }
        ...
        [[nodiscard]] constexpr auto operator+( int const jump_amount ) const -> cell_pos
        {
        ...
        friend constexpr auto operator==( cell_pos const&, cell_pos const& ) -> bool = default;
};
...
struct draw_state
{
        ...
        auto write_sequence(
            ...
            cell_type const&                                                  target_cell_state,
            std::optional< std::reference_wrapper< cell_type const > > const& old_cell_state,
            cell_pos const&                                                   target_pos,
            int const                                                         origin_line,
            int const origin_column ) -> void;

private:
        [[nodiscard]] auto is_position_sequence_necessary( cell_pos const& target_pos ) const
            -> bool;
};

template < oof::std_string_type string_type, typename T, typename... Ts >
auto write_ints_into_string( string_type& target, T const& first, Ts const&... rest ) -> void;

template < oof::std_string_type string_type >
[[nodiscard]] auto get_index_color_seq_str( set_index_color_sequence const& sequence )
    -> string_type;

template < std_string_type string_type >
using fitting_char_sequence_t =
    std::conditional_t< std::is_same_v< string_type, std::string >, char_sequence, wchar_sequence >;
...
template < oof::sequence_c sequence_type >
constexpr auto oof::detail::get_sequence_string_size( sequence_type const& sequence ) -> size_t
{
        constexpr auto get_int_param_str_length = []( int const param ) -> int {
                if ( param < 10 )
                        return 1;
                if ( param < 100 )
                        return 2;
        ...
        if constexpr ( is_any_of< sequence_type, char_sequence, wchar_sequence > ) {
                ...
                constexpr auto get_component_str_size = []( uint8_t const component ) {
                ...
                if constexpr ( is_any_of<
                                   ...
                                   bg_rgb_color_sequence > ) {
                        reserve_size +=
                            2 + semicolon_size + 1 + semicolon_size +
                            get_int_param_str_length( sequence.m_color.red ) + semicolon_size +
                            get_int_param_str_length( sequence.m_color.green ) + semicolon_size +
...
template < typename stream_type, oof::sequence_c sequence_type >
auto oof::operator<<( stream_type& os, sequence_type const& sequence ) -> stream_type&
{
...
template < oof::std_string_type string_type, typename T, typename... Ts >
auto oof::detail::write_ints_into_string( string_type& target, T const& first, Ts const&... rest )
    -> void
{
...
template < oof::std_string_type string_type, oof::sequence_c sequence_type >
auto oof::write_sequence_into_string( string_type& target, sequence_type const& sequence ) -> void
{
        if constexpr ( std::is_same_v<
                ...
                if constexpr ( std::same_as< sequence_type, set_index_color_sequence > )
                    ...
                    std::same_as< sequence_type, store_position_sequence > ||
                    std::same_as< sequence_type, load_position_sequence > ) {
                ...
                if constexpr ( std::is_same_v< sequence_type, fg_rgb_color_sequence > ) {
                        ...
                        target += static_cast< char_type >( 'm' );
                                          ...
                                          is_same_v< sequence_type, cursor_visibility_sequence > ) {
                        target += static_cast< char_type >( '?' );
...
template < oof::std_string_type string_type >
auto oof::detail::get_index_color_seq_str( set_index_color_sequence const& sequence ) -> string_type
{
        using char_type = typename string_type::value_type;
        ...
        auto const write_nibble = [&]( int const nibble ) {
                if ( nibble < 10 )
                        result += static_cast< char_type >( '0' + nibble );
                else
                        result += static_cast< char_type >( 'a' + nibble - 10 );
        };
        auto const write_component = [&]( uint8_t const component ) {
                if ( component > 15 )
...
auto oof::screen< string_type >::update_sequence_buffer() const -> void
{
        ...
        for ( detail::cell_pos relative_pos{ this->m_width, this->m_height };
              relative_pos.is_end() == false;
              ++relative_pos ) {
                cell< string_type > const& target_cell_state = this->m_cells[relative_pos.m_index];

                std::optional< std::reference_wrapper< cell< string_type > const > > old_cell_state;
...
oof::screen< string_type >::screen(
    int const                  width,
    int const                  height,
    int const                  start_column,
    int const                  start_line,
    cell< string_type > const& background )
  : m_width( width )
  , m_height( height )
...
{
        if ( width <= 0 ) {
                std::string const msg = "Width can't be negative";
                ::oof::detail::error( msg );
        }
        if ( height <= 0 ) {
                std::string const msg = "Height can't be negative";
                ::oof::detail::error( msg );
        }
}
...
template < oof::std_string_type string_type >
oof::screen< string_type >::screen( int const width, int const height, char_type const fill_char )
    ...
    cell_format const& formatting ) -> void
{
        ...
        int const ending_column = column + static_cast< int >( text.size() );
...
template < oof::std_string_type string_type >
auto oof::screen< string_type >::is_inside( int const column, int const line ) const -> bool
{
...
template < oof::std_string_type string_type >
auto oof::screen< string_type >::get_cell( int const column, int const line )
    -> cell< string_type >&
{
        if ( line < 0 || line >= m_height ) {
                std::string msg = "Line is out of range. Height is ";
        ...
        int const index = line * m_width + column;
...
auto oof::get_string_reserve_size( std::vector< sequence_variant_type > const& sequences ) -> size_t
{
        size_t reserve_size{};
        for ( sequence_variant_type const& sequence : sequences )
                std::visit(
                    [&]( auto const& alternative ) {
                            reserve_size += detail::get_sequence_string_size( alternative );
                    },
                    sequence );
        return reserve_size;
}
    ...
    string_type&                                target ) -> void
{
        for ( sequence_variant_type const& sequence : sequences )
                std::visit(
                    [&]( auto const& alternative ) {
                            write_sequence_into_string( target, alternative );
                    },
                    sequence );
}
...
template < oof::std_string_type string_type >
auto oof::get_string_from_sequences( std::vector< sequence_variant_type > const& sequences )
    -> string_type
{
...
template auto oof::get_string_from_sequences(
    std::vector< sequence_variant_type > const& sequences ) -> std::string;
template auto oof::get_string_from_sequences(
    std::vector< sequence_variant_type > const& sequences ) -> std::wstring;


auto oof::position( int const line, int const column ) -> position_sequence
{
...
auto oof::vposition( int const line ) -> vposition_sequence
{
        return vposition_sequence{ .m_line = static_cast< uint8_t >( line ) };
}


auto oof::hposition( int const column ) -> hposition_sequence
{
...
auto oof::move_left( int const amount ) -> move_left_sequence
{
        return move_left_sequence{ .m_amount = static_cast< uint8_t >( amount ) };
}


auto oof::move_right( int const amount ) -> move_right_sequence
{
        return move_right_sequence{ .m_amount = static_cast< uint8_t >( amount ) };
}


auto oof::move_up( int const amount ) -> move_up_sequence
{
        return move_up_sequence{ .m_amount = static_cast< uint8_t >( amount ) };
}


auto oof::move_down( int const amount ) -> move_down_sequence
{
        return move_down_sequence{ .m_amount = static_cast< uint8_t >( amount ) };
}


auto oof::fg_color( color const& col ) -> fg_rgb_color_sequence
{
        return fg_rgb_color_sequence{ .m_color = col };
}


auto oof::fg_color( int const index ) -> fg_index_color_sequence
{
...
auto oof::set_index_color( int const index, color const& col ) -> set_index_color_sequence
{
        if ( index < 1 || index > 255 ) {
                std::string msg = "Index must be in [1, 255], was: ";
                msg += std::to_string( index );
...
auto oof::bg_color( color const& col ) -> bg_rgb_color_sequence
{
        return bg_rgb_color_sequence{ .m_color = col };
}


auto oof::bg_color( int const index ) -> bg_index_color_sequence
{
        if ( index < 1 || index > 255 ) {
...
auto oof::underline( bool const new_value ) -> underline_sequence
{
        return underline_sequence{ .m_underline = new_value };
}


auto oof::bold( bool const new_value ) -> bold_sequence
{
        return bold_sequence{ .m_bold = new_value };
}


auto oof::cursor_visibility( bool const new_value ) -> cursor_visibility_sequence
{
...
oof::pixel_screen::pixel_screen(
    int const    width,
    int const    halfline_height,
    int const    start_column,
    int const    start_halfline,
    color const& fill_color )
...
oof::pixel_screen::pixel_screen( int const width, int const halfline_height )
...
auto oof::pixel_screen::compute_result() const -> void
{
        ...
        for ( int line = 0; line < m_screen.get_height(); ++line ) {
                for ( int column = 0; column < m_screen.get_width(); ++column ) {
                        ...
                        target_cell.m_format.m_fg_color   = is_in( column, halfline_top ) ?
                                                                get_color( column, halfline_top ) :
                                                                m_fill_color;
                        target_cell.m_format.m_bg_color   = is_in( column, halfline_bottom ) ?
                                                                get_color( column, halfline_bottom ) :
                                                                m_fill_color;
                }
                halfline_top += 2;
                halfline_bottom += 2;
...
{
        int const first_line = m_origin_halfline / 2;
        int const last_line  = ( m_origin_halfline - 1 + m_halfline_height ) / 2;
        return last_line - first_line + 1;
}


auto oof::pixel_screen::is_in( int const column, int const halfline ) const -> bool
{
        size_t const index = halfline * this->get_width() + column;
        return index >= 0 && index < m_pixels.size();
}


auto oof::pixel_screen::get_color( int const column, int const halfline ) const -> color const&
{
        size_t const index = halfline * this->get_width() + column;
...
auto oof::pixel_screen::get_color( int const column, int const halfline ) -> color&
{
        size_t const index = halfline * this->get_width() + column;
        return m_pixels[index];
}
    ...
    int const                                                         origin_column ) -> void
{
        ...
        if ( m_format.has_value() == false ) {
                sequence_buffer.push_back(
                    fg_rgb_color_sequence{ .m_color = target_cell_state.m_format.m_fg_color } );
                sequence_buffer.push_back(
                    bg_rgb_color_sequence{ .m_color = target_cell_state.m_format.m_bg_color } );
                sequence_buffer.push_back(
                    underline_sequence{ .m_underline = target_cell_state.m_format.m_underline } );
                sequence_buffer.push_back(
                    bold_sequence{ .m_bold = target_cell_state.m_format.m_bold } );
        } else {
                // Apply differences between console state and the target state
                if ( target_cell_state.m_format.m_fg_color != m_format->m_fg_color )
                        sequence_buffer.push_back( fg_rgb_color_sequence{
                            .m_color = target_cell_state.m_format.m_fg_color } );
                if ( target_cell_state.m_format.m_bg_color != m_format->m_bg_color )
                        sequence_buffer.push_back( bg_rgb_color_sequence{
                            .m_color = target_cell_state.m_format.m_bg_color } );
                if ( target_cell_state.m_format.m_underline != m_format->m_underline )
                        sequence_buffer.push_back( underline_sequence{
                            .m_underline = target_cell_state.m_format.m_underline } );
        ...
        if ( this->is_position_sequence_necessary( target_pos ) ) {
                sequence_buffer.push_back( position_sequence{
        ...
        sequence_buffer.push_back(
            fitting_char_sequence_t< string_type >{ .m_letter = target_cell_state.m_letter } );
...
auto oof::detail::draw_state< string_type >::is_position_sequence_necessary(
    cell_pos const& target_pos ) const -> bool
{
        // There is was nothing written before, hence the cursor position is unknown
        if ( m_last_written_pos.has_value() == false )
                return true;

        cell_pos const current_cursor_pos = m_last_written_pos.value() + 1;

        // The cursor position is known to be wrong
        if ( target_pos != current_cursor_pos )
                return true;

        // If we're on the "right" position according to the subset of the buffer, the position
        // still needs to be set if there was a line jump.
...
template < oof::std_string_type string_type, oof::sequence_c sequence_type >
auto oof::get_string_from_sequence( sequence_type const& sequence ) -> string_type
{
        string_type result{};
...
auto oof::detail::error( std::string const& msg ) -> void
{
...
auto oof::detail::get_pixel_background( color const& fill_color ) -> cell< std::wstring >
{
        return cell< std::wstring >{
            .m_letter = L'▀', .m_format = { .m_fg_color = fill_color, .m_bg_color = fill_color } };
}
...
{
        sequence_type const& sequence = static_cast< sequence_type const& >( *this );
...
template < typename sequence_type >
auto oof::detail::extender< sequence_type >::operator+( std::string const& other ) const
    -> std::string
{
        sequence_type const& sequence = static_cast< sequence_type const& >( *this );
...
template < typename sequence_type >
auto oof::detail::extender< sequence_type >::operator+( std::wstring const& other ) const
    -> std::wstring
{
        sequence_type const& sequence = static_cast< sequence_type const& >( *this );
...
auto impl_fun() -> void
{
        std::visit(
            []( auto const& altern ) {
                    return altern + std::string{};
            },
            oof::sequence_variant_type{} );
        std::visit(
            []( auto const& altern ) {
                    return altern + std::wstring{};
            },
            oof::sequence_variant_type{} );
}
